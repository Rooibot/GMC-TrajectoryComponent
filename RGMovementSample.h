...
#include "RGMovementSample.generated.h"

USTRUCT(BlueprintType)
struct ROOICORE_API FRGMovementSample
{
	GENERATED_BODY()
	...
	FRGMovementSample(const FTransform& TransformWorldSpace, const FVector& LinearVelocity)
	{
	...
	FRotator GetRotationVelocityFrom(const FRGMovementSample& OtherSample) const
	{
	...
	FVector GetAccelerationFrom(const FRGMovementSample& OtherSample) const
	{
	...
	float DistanceFrom(const FRGMovementSample& OtherSample) const
	{
	...
	void operator =(const FRGMovementSample& Other)
	{
	...
	bool Equals(const FRGMovementSample& Other, const bool bIgnoreTime = false) const
	{
	...
	bool operator ==(const FRGMovementSample& Other) const
	{
		return Equals(Other);
	}
...
USTRUCT(BlueprintType)
struct ROOICORE_API FRGMovementSampleCollection
{
	GENERATED_BODY()

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
	TArray<FRGMovementSample> Samples;
	...
	explicit operator FTrajectorySampleRange() const
	{
		...
		for (const FRGMovementSample& Sample : Samples)
		{
